...
//      // coding bits 1110 Message Waiting Indication Group: Store Message
...
//      // coding bits 1111 Data coding/message class
...
//  };
  ...
  public byte mapDcs(final byte tpDcs) throws InvalidDataCodingSchemeException {
    if (tpDcs == (byte) 0x00) {
      return 0x00;
    }
    // coding bits 00xx
    if ((tpDcs & (byte) 0xc0) == (byte) 0x00) {
      // no message class or message class 0
      if ((tpDcs & (byte) 0x10) == (byte) 0x00 || (tpDcs & (byte) 0x02) == (byte) 0x00) {
        switch (tpDcs & (byte) 0x0c) {
          ...
          case 0x0c:
            // reserved
            throw new InvalidDataCodingSchemeException("Reserved TP-DCS " + String.format("%02X", tpDcs));
        }
      }
      return (byte) (tpDcs | (byte) 0xf0);
    }
    // coding bits 110x: Message Waiting Indication
    if ((tpDcs & (byte) 0xe0) == (byte) 0xc0) {
      return tpDcs;
    }
    // coding bits 1111
    if ((tpDcs & (byte) 0xf0) == (byte) 0xf0) {
      return tpDcs;
    }
    throw new InvalidDataCodingSchemeException("Reserved TP-DCS " + String.format("%02X", tpDcs));
  }
